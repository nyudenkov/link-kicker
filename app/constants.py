...
class Message(StrConstants):
    ...
    MAILING_OFF = _(
        "🥲 Ежедневная рассылка отключена, не забывай про меня! Можешь "
        "использовать /random для получения случайной ссылки или /links для "
        "просмотра списка еще непрочитанных"
    )
    MAILING_ON = _(
        "🥰 Ежедневная рассылка включена! Кстати, можно использовать /random "
        "для получения случайной ссылки или /links для просмотра списка еще "
        "непрочитанных"
    )
...
У тебя как и у меня есть проблема с тем, что ты забываешь прочесть сохранённые в
закладках статьи?
...
Появилось желание прочесть что-то из добавленных ссылок, но не хочешь ждать
наступления времени рассылки?
