...
async def on_send_location_clicked(
    m: CallbackQuery, dialog: Dialog, manager: DialogManager
):
    ...
    manager.current_context().dialog_data["location_request_msg_id"] = (
        message.message_id
    )


async def delete_location_request_message(
    callback: CallbackQuery, dialog: Dialog, manager: DialogManager
):
...
async def on_location_sent(message: Message, dialog: Dialog, manager: DialogManager):
    ...
    timezone_str = TimezoneFinder().certain_timezone_at(
        lat=location.latitude, lng=location.longitude
    )

    timezone_utc_offset = (
        datetime.datetime.now(pytz.timezone(timezone_str)).utcoffset().total_seconds()
        / 60
        / 60
    )
...
async def save_timezone_on_offset_input(
    m: Message, dialog: Dialog, manager: DialogManager
):
...
timezone_dialog = Dialog(
    Window(
        IConst(
            _(
                "Отправь свою локацию чтобы определить часовой пояс или выбери "
                "вручную\n"
                "<i>В случае отправления локации боту, данные будут использованы "
                "только единожды для определения часового пояса и не будут "
                "сохранены.</i>"
            )
        ),
        Group(
    ...
    Window(
        IConst(
            _(
                ...
                "<a href='https://www.timeanddate.com/time/difference/timezone/utc'>"
                "этот сайт</a>"
            )
        ),
        SwitchTo(
    ...
    Window(
        IConst(
            _(
                "Произошла ошибочка! На примере с Москвой - надо написать просто "
                "<b>+3</b>; с Нью-Йорком - <b>-4</b>"
            )
        ),
        MessageInput(save_timezone_on_offset_input),
