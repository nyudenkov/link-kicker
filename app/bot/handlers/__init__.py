...
def setup(dp: Dispatcher):
    ...
    dp.register_message_handler(
        statistics.bot_statistics_handler, commands="botstatistics"
    )

    dp.register_message_handler(
        link.message_link_handler, lambda m: m.text and m.text[0] != "/"
    )
    ...
    dp.register_callback_query_handler(
        link.links_page_handler,
        lambda c: c.data and c.data.startswith("links_paginator"),
    )
    dp.register_callback_query_handler(
        link.del_link_from_links_handler,
        lambda c: c.data and c.data.startswith("links_del"),
    )
